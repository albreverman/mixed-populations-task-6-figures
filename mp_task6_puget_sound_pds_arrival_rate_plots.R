...
spaghetti_plot_arrival_rate <- function(eventCountDfByFloodMechanism, regionName, thisMechanism, aspectLabel, xAxisBreaks, xAxisLabels, meanRate) {
  ...
  spaghetti <- ggplot() +
    ...
    labs(color = "Gage ID") +
    ggtitle(paste0(regionName, " ", aspectLabel, ": ", thisMechanism)) +
...
inputFile <- "C:/Projects/MixedPopulations/TASK5/PugetSound/PugetSoundPDSEventCountSummary_10years.csv"
outputDir <- "C:/Projects/MixedPopulations/TASK6/PugetSound/PDS_Figures/"
...
easternSlopeCascades <- c("12488500", "12457000", "12456500", "12452990", "12451000", "12448998", "12447390")
...
eventCountDf <- read.csv(inputFile) %>%
  mutate(GageId = as.character(GageId),
    ...
    Rate = ifelse(NumberYears > minNumberYearsDailyData, Count/NumberYears, NA),
    Aspect = "West")

eventCountDf$Aspect[eventCountDf$GageId %in% easternSlopeCascades] <- "East"

floodMechanisms <- unique(eventCountDf$FloodMechanism)
aspects <- unique(eventCountDf$Aspect)

# Create and save spaghetti plot of arrival rates for each flood mechanism
for (i in seq(length(floodMechanisms))) {
  for (j in seq(length(aspects) + 1)) {
    # Subset dataframe to current flood mechanism and current aspect
    thisMechanism <- floodMechanisms[i]
    thisAspect <- ifelse(j == 3, "All Gages", aspects[j])
    
    eventCountDfByFloodMechanism <- eventCountDf[eventCountDf$FloodMechanism == thisMechanism &
                                                   eventCountDf$Aspect == thisAspect, ]
    ...
    colors <- setNames(scales::pal_hue()(length(gageIds)), gageIds[-length(gageIds) +
                                                                     1])
    colors["Average"] <- "black"
    aspectLabel <- ifelse(j == 3, "All Gages", paste0(thisAspect, "ern Slopes"))
    
    # Generate spaghetti plot of arrival rate
    spaghetti_plot <-  spaghetti_plot_arrival_rate(
      eventCountDfByFloodMechanism,
      regionName,
      thisMechanism,
      aspectLabel,
      xAxisBreaks,
    ...
    figureName <- paste0(regionName, timePeriod,
                         " Spaghetti Plot Arrival Rate ",
                         thisMechanism,
                         " ",
                         thisAspect)
...
for (j in seq(length(aspects) + 1)) {
  ...
  eventCountDfAllFloodMechanisms <- eventCountDf[eventCountDf$Aspect == thisAspect, ]
  ...
  eventCountDfAllFloodMechanisms <- eventCountDfAllFloodMechanisms %>%
  ...
  spaghetti_plot_all_gages <- ggplot() +
    ...
    ylab("Arrival Rate (events/year)") +
    labs(color = "Gage ID") +
    ggtitle(paste0(regionName, " ", aspectLabel, "All Flood Mechanisms")) +
    scale_x_continuous(breaks = xAxisBreaks, labels = xAxisLabels) +
    theme_bw()
  spaghetti_plot_all_gages
  
  figureName <- paste0(regionName, timePeriod, " Spaghetti Plot Arrival Rate All Floods ", thisAspect)
...
for (j in seq(length(aspects) + 1)) {
  ...
  thisAspect <- ifelse(j == 3, "All Gages", aspects[j])
  thisMeanRate <- meanRate[meanRate$Aspect == thisAspect, ]
  if (j == 3) {
    thisMeanRate <- thisMeanRate
  }
  
  aspectLabel <- ifelse(j == 3, "All Gages", paste0(thisAspect, "ern Slopes"))
  
  line_plot_average_all_mechanisms <- ggplot(thisMeanRate, mapping = aes(x = as.numeric(StartDate), y = Rate, color = as.factor(FloodMechanism))) +
    geom_line(size = 1) +
    ...
    ggtitle(paste0(regionName, " ", aspectLabel)) +
    theme_bw()
  line_plot_average_all_mechanisms
  
  figureName <- paste0(regionName, timePeriod, " Line Plot Average Arrival Rate By Flood Mechanism ", thisAspect)
...
for (j in seq(length(aspects) + 1)) {
  ...
  thisAspect <- ifelse(j == 3, "All Gages", aspects[j])
  eventCountDfThisAspect <- eventCountDf[eventCountDf$Aspect == thisAspect, ]
  if (j == 3) {
    eventCountDfThisAspect <- eventCountDf
  }
  
  facetLabels <- c()
  
  # Get mean number of years of daily data for gages in each time period
  meanNumberOfYears <- eventCountDfThisAspect %>%
    group_by(StartDate) %>%
  ...
  eventCountDfThisAspect <- eventCountDfThisAspect %>%
    mutate(StartDateLabel = "")
  
  eventCountDfThisAspect$StartDateLabel <- factor(eventCountDfThisAspect$StartDate, labels = facetLabels)
  aspectLabel <- ifelse(j == 3, "All Gages", paste0(thisAspect, "ern Slopes"))
  
  # Facet bar chart
  barChartByTimePeriod <- ggplot(eventCountDfThisAspect, aes(x = FloodMechanism, y = Rate, fill = FloodMechanism)) + 
    ...
    labs(fill = "Flood Mechanism",
         title = paste0(regionName, " ", aspectLabel)) +
    xlab("") +
    ylab("Arrival Rate (events/year)")
  barChartByTimePeriod
  
  figureName <- paste0(regionName, timePeriod, " Bar Chart of Flood Mechanism Arrival Rate By Time Period ", thisAspect)
...
for (j in seq(length(aspects) + 1)) {
  ...
  floodTypeCounts <- eventCountDfThisAspect %>%
  ...
  nCol <- ceiling(length(startDates) / 2)
  # Arrange plots in grid
  arrangedPieCharts <- do.call(ggarrange, c(pieChartList, nrow = 2, ncol = nCol, common.legend = TRUE, legend = "right")) 
  
  # Add a title
  pieChartTitle <- paste0(
    regionName, " ",
    aspectLabel,
    " Partial Duration Series Flood Mechanisms By Time Period"
  )
  
  arrangedPieCharts <- annotate_figure(arrangedPieCharts,
                                       top = text_grob(
                                         pieChartTitle,
                                         color = "black",
                                         face = "bold",
  ...
  figureName <- paste0(regionName, timePeriod, " PDS Pie Chart ", thisAspect)
