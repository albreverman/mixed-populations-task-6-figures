...
inputFile <- "C:/Projects/MixedPopulations/TASK5/RedRiveroftheNorth/RedRiverPDSEventCountSummary_20years.csv"
outputDir <- "C:/Projects/MixedPopulations/TASK6/RedRiveroftheNorth/PDS_Figures/"
regionName <- "Red River of the North"
timePeriod <- " 20 years "
...
eventCountDf <- read.csv(inputFile) %>%
  ...
  mutate(FloodMechanism = recode(FloodMechanism, Rainfall = "Rain-Only", Snowmelt = "Snowmelt-Only"))

eventCountDf <- eventCountDf[eventCountDf$EndDate < as.Date("2020-01-01") & eventCountDf$Start > as.Date("1909-12-31"), ]
...
line_plot_average_all_mechanisms <- ggplot(meanRate, mapping = aes(x = as.numeric(StartDate), y = Rate, color = as.factor(FloodMechanism))) +
  ...
  ggtitle(regionName) +
  ylim(0, 1) + # Might need to change upper limit
  theme_bw()
